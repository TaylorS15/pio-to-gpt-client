...
export const useStore = create<AppState>()((set) => ({
	...
	setConversation: (currentConversation) => set(() => ({ currentConversation })),
	...
	updateResponse: (response) =>
		set((state) => {
			if (state.currentConversation === null) {
				...
				const lastQuestionIndex = state.currentConversation.conversation.length - 1;
	...
	addConversation: (conversation) =>
		set((state) => {
			if (state.pastConversations === null) {
				...
				return { pastConversations: [...state.pastConversations, conversation] };
			}
		}),
	updateConversation: (conversation) =>
		set((state) => {
			if (state.pastConversations === null) {
				...
				const updatedPastConversations = state.pastConversations.map((pastConversation) => {
